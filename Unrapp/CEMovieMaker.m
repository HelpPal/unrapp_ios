...
- (instancetype)initWithSettings:(NSDictionary *)videoSettings andFileName:(NSString *)fileName
{
    self = [self init];
    if (self) {
        NSError *error;
        ...
        NSString *tempPath = fileName; //[documentsDirectory stringByAppendingFormat:@"/export.mov"];
...
+ (NSDictionary *)videoSettingsWithCodec:(NSString *)codec withWidth:(CGFloat)width andHeight:(CGFloat)height
{
    ...
    if ((int)width % 16 != 0 ) {
        NSLog(@"Warning: video settings width must be divisible by 16...updating!");
        while ((int)width % 16 != 0)
        {
            width = width + 1.0;
        }
    }
